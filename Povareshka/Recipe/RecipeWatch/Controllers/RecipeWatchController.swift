    ...
    private func loadRecipeData() {
        ...
        Task {
            do {
                ...
                async let ratings = loadRatingsData(recipeId: recipeId)

                
                let (recipeResult, ingredientsResult, instructionsResult, tagsResult, categoriesResult, (avgRating, userRating)) =
                await (try recipe, try ingredients, try instructions, try tags, try categories, try ratings)
...
//    }
    ...
    private func loadRatingsData(recipeId: UUID) async throws -> (Double, Rating?) {
        async let avgRating = RatingManager.shared.getAverageRating(for: recipeId)
        async let userRating = RatingManager.shared.getUserRating(for: recipeId)
        return await (try avgRating, try userRating)
    }
    ...
    private func updateUI() {
        ...
        if let imagePath = recipe.imagePath {
            Task {
                do {
                    ...
                    let favorite: Bool
                    do {
                        async let creatorTask = self.checkIfCurrentUserIsCreator(recipe: recipe)
                        async let favoriteTask = FavoriteRecipeManager.shared.isRecipeFavorite(recipeId: recipe.id)
                        creator = await creatorTask
                        favorite = try await favoriteTask
                    } catch {
                        print("Ошибка при проверке статуса: \(error)")
                        creator = false
                        favorite = false
                    }
                    ...
                    DispatchQueue.main.async {
                        ...
                        self.recipeImageView.favoriteStatusChanged = {
                            // Можно обновить UI или выполнить другие действия при изменении статуса
                            print("Статус избранного изменился: \($0)")
                        }
        ...
        metaStack.configure(with: recipe, averageRating: averageRating, recipeId: recipe.id) // Rating
        ...
        ingredientTableView.layer.cornerRadius = 15
